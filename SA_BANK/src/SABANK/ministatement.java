...
public class ministatement extends JFrame implements ActionListener{
    ...
    ministatement(String pinnumber){
        ...
        try{
            ...
            ResultSet rs = c.stmt.executeQuery("select * from login where pin = '"+pinnumber+"'");
            while(rs.next()){
        ...
        try{
            int balance = 0;
            datacon c1  = new datacon();
            ResultSet rs = c1.stmt.executeQuery("SELECT * FROM bank where pin = '"+pinnumber+"'");
            while(rs.next()){
